...
def build_training_objects(config: RunConfig, device: str):
    ...
    if config.model_type == "big":
        model = ModelBig(backbone, 100, dropout=config.dropout).to(device)
    elif config.model_type == "small":
        model = ModelSmall(backbone, 100, dropout=config.dropout).to(device)
    elif config.model_type == "narrow":
        model = ModelNarrow(backbone, 100, dropout=config.dropout).to(device)
    else:
        model = ModelWide(backbone, 100, dropout=config.dropout).to(device)
...
def train_model(model, criterion, optimizer, scheduler, config: RunConfig, train_dataloader: DataLoader, test_dataloader: DataLoader, device: str):
    ...
    for epoch in range(config.num_epochs):
        ...
        test_acc, test_loss, top5_acc = evaluate_model(model, test_dataloader, device)
        avg_loss = total_loss / len(train_dataloader)
        current_lr = optimizer.param_groups[0]["lr"]

        if test_acc > best_acc:
            best_acc = test_acc
            torch.save(model.state_dict(), f"{dir_path}/d1.pth")

        if scheduler is not None:
            scheduler.step()

        print(f"[{config.name}] Epoch {epoch + 1}/{config.num_epochs}: Loss={avg_loss:.4f}, Train={train_acc:.2f}%, Test={test_acc * 100:.2f}%, Top5={top5_acc * 100:.2f}%, LR={current_lr:.6f}")
...
def evaluate_model(model: ModelBig | ModelSmall, test_dataloader: DataLoader, device: str):
    ...
    return total_correct / n, test_loss / n, top5_correct / n


def save_summary(config: RunConfig, train_acc, test_acc, loss_history, lr_history, top5_acc):
    ...
    save_dir = f"{os.path.dirname(__file__)}/D1/{config.name}"
...
RUNS = [
    RunConfig(name="rbest", model_type="big", dropout=[0.5, 0.4], optim_type="adam", lr=1e-3, weight_decay=1e-3, scheduler_type="cosine", label_smoothing=0.1, batch_size=32, note="best"),
    # Scheduler
    ...
    # # Batch size
    # RunConfig(name="r07_bs32", model_type="big", dropout=[0.2, 0.1], optim_type="adam", lr=1e-3, weight_decay=1e-3, scheduler_type="cosine", label_smoothing=0.1, batch_size=32, note="batch size 32"),
    # RunConfig(name="r08_bs64", model_type="big", dropout=[0.2, 0.1], optim_type="adam", lr=1e-3, weight_decay=1e-3, scheduler_type="cosine", label_smoothing=0.1, batch_size=64, note="batch size 64"),
    # RunConfig(name="r09_bs128", model_type="big", dropout=[0.2, 0.1], optim_type="adam", lr=1e-3, weight_decay=1e-3, scheduler_type="cosine", label_smoothing=0.1, batch_size=128, note="batch size 128"),
    # # Learning rate
    ...
    # # Dropout
    # RunConfig(name="r10_dropout_low", model_type="big", dropout=[0.1, 0.05], optim_type="adam", lr=1e-3, weight_decay=1e-3, scheduler_type="cosine", label_smoothing=0.1, batch_size=32, note="dropout: very low"),
    # RunConfig(name="r11_dropout_mid", model_type="big", dropout=[0.3, 0.2], optim_type="adam", lr=1e-3, weight_decay=1e-3, scheduler_type="cosine", label_smoothing=0.1, batch_size=32, note="dropout: medium"),
    # RunConfig(name="r12_dropout_high", model_type="big", dropout=[0.5, 0.4], optim_type="adam", lr=1e-3, weight_decay=1e-3, scheduler_type="cosine", label_smoothing=0.1, batch_size=32, note="dropout: high"),
    # # Label smoothing
    # RunConfig(name="r13_smooth_0", model_type="big", dropout=[0.5, 0.4], optim_type="adam", lr=1e-3, weight_decay=1e-3, scheduler_type="cosine", label_smoothing=0.0, batch_size=32, note="no label smoothing"),
    # RunConfig(name="r14_smooth_02", model_type="big", dropout=[0.5, 0.4], optim_type="adam", lr=1e-3, weight_decay=1e-3, scheduler_type="cosine", label_smoothing=0.2, batch_size=32, note="label smoothing 0.2"),
    # SHOULD have been 0.3 0.2
    # RunConfig(name="r18_smooth_0_less_dropout", model_type="big", dropout=[0.5, 0.4], optim_type="adam", lr=1e-3, weight_decay=1e-3, scheduler_type="cosine", label_smoothing=0.0, batch_size=32, note="no label smoothing"),
    # RunConfig(name="r19_smooth_02_less_dropout", model_type="big", dropout=[0.3, 0.2], optim_type="adam", lr=1e-3, weight_decay=1e-3, scheduler_type="cosine", label_smoothing=0.2, batch_size=32, note="label smoothing 0.2"),
    # # Architecture
    # RunConfig(name="r16_narrow", model_type="narrow", dropout=[0.2, 0.1], optim_type="adam", lr=1e-3, weight_decay=1e-3, scheduler_type="cosine", label_smoothing=0.1, batch_size=32, note="narrow: 576->256->100"),
    # RunConfig(name="r17_wide", model_type="wide", dropout=[0.2, 0.1, 0.1], optim_type="adam", lr=1e-3, weight_decay=1e-3, scheduler_type="cosine", label_smoothing=0.1, batch_size=32, note="wide: 576->2048->1024->512->100"),
    # RunConfig(name="r21_narrow", model_type="narrow", dropout=[0.3, 0.2], optim_type="adam", lr=1e-3, weight_decay=1e-3, scheduler_type="cosine", label_smoothing=0.1, batch_size=32, note="narrow: 576->256->100"),
    # RunConfig(name="r22_wide", model_type="wide", dropout=[0.3, 0.2, 0.1], optim_type="adam", lr=1e-3, weight_decay=1e-3, scheduler_type="cosine", label_smoothing=0.1, batch_size=32, note="wide: 576->2048->1024->512->100"),
    # RunConfig(name="r15_small", model_type="small", dropout=0.2, optim_type="adam", lr=1e-3, weight_decay=1e-3, scheduler_type="cosine", label_smoothing=0.1, batch_size=32, note="small: 576->512->100"),
...
if __name__ == "__main__":
    ...
    for config in RUNS:
        ...
        model.load_state_dict(torch.load(f"{os.path.dirname(__file__)}/models/d1.pth", weights_only=True, map_location=device))
        _, _, top5_acc = evaluate_model(model, data.test_dataloader, device)
        ...
        save_summary(config, train_acc, test_acc, loss_history, lr_history, top5_acc)
